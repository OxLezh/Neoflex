...
def exist_table(table_name, engine, schema):
    ...
    with engine.connect() as conn:
        ...
        conn.close()
        if not exist_table:
            logging(table_name, 'error_exist_object', engine,
                    f'Таблицы {schema}.{table_name} не существует.')
            print(
                f"Таблицы c данными {schema}.{table_name} в PostgreSQL не существует.\n")
    return exist_table
...
def extract_data(table_name, engine, path):
    ...
    try:
        ...
        if table_name == 'ft_posting_f':
            df = df.groupby(['oper_date', 'credit_account_rk', 'debet_account_rk'])[
                ['credit_amount', 'debet_amount']].sum().reset_index()
        ...
        logging(table_name, 'error_extract', engine,
                f'ОШИБКА извлечения данных из файла {table_name}.csv', str(sys.exc_info()).replace("'", ''))
        print(
            f'Ошибка извлечения данных из файла {table_name}.csv: \n {sys.exc_info()}')
...
def upload_data(table_name, engine, df):
    ...
    """
    try:
        logging(table_name, 'start_upload', engine,
                f'Старт загрузки данных в таблицу {table_name}')
        time.sleep(2)
        with engine.connect() as conn:
            ...
            columns_no_pk_list = (
                [item for item in column_list if item not in column_pk_list])
            columns_no_pk = ", ".join(columns_no_pk_list)
            columns_excluded = ", ".join(
                ['excluded.' + direction for direction in columns_no_pk_list])
            ...
            conn.commit()
            conn.close()
            logging(table_name, 'end_upload', engine,
                    f'Данные в таблицу {table_name} загружены.')
            print(f'Данные в таблицу {table_name} загружены.\n')
    except:
        logging(table_name, 'error_upload', engine, f'ОШИБКА загрузки данных в таблицу {table_name}', str(
            sys.exc_info()).replace("'", ''))
        print("Ошибка загрузки данных в таблицу", table_name,
